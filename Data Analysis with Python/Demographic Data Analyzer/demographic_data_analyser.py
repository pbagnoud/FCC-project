...
def calculate_demographic_data(print_data=True):
    ...
    race_count = pd.value_counts(df["race"], sort=False)
    ...
    for country, group in df.groupby("native-country"):
        rich_current = round(group[group["salary"]==">50K"].shape[0]/group.shape[0]*100,1)
        if rich_current > highest_earning_country_percentage:
            highest_earning_country = country
            highest_earning_country_percentage = rich_current

    # Identify the most popular occupation for those who earn >50K in India.

    top_IN_occupation = df[(df["native-country"]=="India") & (df["salary"] ==">50K")].value_counts(df["occupation"]).idxmax()
